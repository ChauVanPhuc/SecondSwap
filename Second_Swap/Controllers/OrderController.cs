    ...
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> Approved(int id)
        {
            ...
            if (EmailBuyer != null)
            {
                ...
                {
                    await _emailService.SendEmailAsync(EmailBuyer, "Approved Order", order.Buyer.FullName + " Your order has been Approved");
                }
        ...
        {
            var order = _db.Orders.Include(x => x.Product).ThenInclude(x => x.Category).Include(x => x.Feedbacks).FirstOrDefault(x => x.Id == id);
